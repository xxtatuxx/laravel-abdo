...
let interval: number;
let startX = 0, endX = 0;
let isDragging = false, dragStartX = 0, dragCurrentX = 0, dragOffset = 0;
const transitionEnabled = ref(true);

// ---- دوال السلايدر ----
function nextSlide() { transitionEnabled.value = true; currentSlide.value = (currentSlide.value + 1) % slides.value.length; }
function prevSlide() { transitionEnabled.value = true; currentSlide.value = (currentSlide.value - 1 + slides.value.length) % slides.value.length; }
...
// ---- السحب ----
function handleTouchStart(e: TouchEvent) { transitionEnabled.value = false; startX = e.touches[0].clientX; }
function handleTouchMove(e: TouchEvent) {
  if (!sliderTrack.value) return;
  const currentX = e.touches[0].clientX;
  const diff = startX - currentX;
...
function handleTouchEnd(e: TouchEvent) {
  ...
  else nextTick(() => sliderTrack.value && (sliderTrack.value.style.transform = `translateX(-${currentSlide.value * 100}%)`));
}
function handleMouseDown(e: MouseEvent) {
...
function handleMouseUp() {
  ...
  transitionEnabled.value = true;
  Math.abs(diff) > threshold ? (diff > 0 ? nextSlide() : prevSlide()) : nextTick(() => sliderTrack.value && (sliderTrack.value.style.transform = `translateX(-${currentSlide.value * 100}%)`));
  isDragging = false;
  document.removeEventListener('mousemove', handleMouseMove); document.removeEventListener('mouseup', handleMouseUp);
  interval = setInterval(nextSlide, 4000);
}

// ----- Carousel الحلقات السفلي -----
const page = usePage<{
  episodes: { data: Episode[]; next_page_url?: string; current_page?: number };
  filters?: { search?: string };
}>();
...
let isCarouselDragging = false, carouselStartX = 0, carouselScrollLeft = 0;
function handleCarouselMouseDown(e: MouseEvent) {
  if (!carouselRef.value) return;
  e.preventDefault();
  isCarouselDragging = true;
...
function handleCarouselMouseMove(e: MouseEvent) {
  if (!isCarouselDragging || !carouselRef.value) return;
  e.preventDefault();
  const walk = (e.pageX - carouselStartX) * 1.5;
  carouselRef.value.scrollLeft = carouselScrollLeft - walk;
}
function handleCarouselMouseUp() {
  if (!carouselRef.value) return;
...
const onCarouselScroll = () => {
  const el = carouselRef.value;
  if (!el) return;
  if (el.scrollLeft + el.clientWidth >= el.scrollWidth - 50) loadMoreEpisodes();
};

// الانتقال للحلقة
const goToEpisode = (id: number) => { inertia.visit(`/episodes/${id}`); };
...
<template>
  <Head title="home" />
  <AppLayout>
    <button @click="toggleDarkMode" class="dark-mode-btn">تبديل وضع ليلي/نهاري</button>
    ...
    <div
      ...
      @mousedown="handleMouseDown"
    >
      <div ref="sliderTrack" class="slider-track" :class="{ 'transition-enabled': transitionEnabled }" :style="{ transform: `translateX(-${currentSlide * 100}%)` }">
      ...
      <button @click="prevSlide" @mousedown.prevent class="slider-nav-btn slider-prev">‹</button>
      <button @click="nextSlide" @mousedown.prevent class="slider-nav-btn slider-next">›</button>
      <div class="slider-indicators">
        <div class="indicators-container">
          <button v-for="(s, i) in slides" :key="i" @click="goToSlide(i)" @mousedown.prevent class="indicator-btn" :class="{ active: i === currentSlide }"></button>
        </div>
      </div>
    </div>
    ...
    <div class="episodes-header">
      <h2 class="episodes-title">آخر الحلقات المضافة</h2>
      <div class="carousel-controls">
        <button @click="scrollLeft" class="carousel-nav-btn">‹</button>
        <button @click="scrollRight" class="carousel-nav-btn">›</button>
      </div>
    </div>

    <!-- Carousel الحلقات -->
    <div ref="carouselRef" @scroll="onCarouselScroll" @mousedown="handleCarouselMouseDown" class="episodes-carousel">
      <div v-for="episode in episodes" :key="episode.id" @click="goToEpisode(episode.id)" class="episode-card">
        <div class="episode-thumbnail">
          <img v-if="episode.thumbnail" :src="`/storage/${episode.thumbnail}`" alt="thumbnail" class="thumbnail-image" />
          <div v-else class="thumbnail-placeholder">No Image</div>
          <div v-if="episode.is_published" class="episode-badge live-badge">يعرض الآن</div>
          <div v-if="episode.video_format" class="episode-badge format-badge">{{ episode.video_format }}</div>
        </div>
        <div class="episode-info">
          <span class="episode-title">{{ episode.title }}</span>
          <span class="episode-number">الحلقة {{ episode.episode_number }}</span>
      ...
      <div v-if="loadingMore" class="loading-skeletons">
        <div v-for="n in 3" :key="n" class="episode-skeleton"></div>
      </div>
    </div>
  </AppLayout>
</template>
...
<style scoped>
/* Dark / Light Mode */
.dark .episode-card { background: #1f2937; color: #f3f4f6; }
.dark .episode-number { color: #9ca3af; }
.dark .slide-content { background: rgba(0,0,0,0.6); }
.dark-mode-btn { position: fixed; top: 1rem; right: 1rem; z-index: 50; padding: 0.5rem 1rem; background: #2563eb; color: white; border-radius: 0.375rem; cursor: pointer; }

/* باقي CSS يبقى كما في الكود السابق */
</style>
